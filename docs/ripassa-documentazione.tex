...
rotazione automatica, ricerca, storico. Il codice \`e coperto da 232 test
...
\texttt{ripassi/ripassiLogic.ts} & classificazione attivi/storico, ordinamento totale, ricerca \\
...
\texttt{shared/currentUser.ts} & id dell'utente autenticato, per le righe con \texttt{user\_id} \\
\midrule
...
\texttt{src/**/\_\_tests\_\_/} & 232 test su 20 suite (dettaglio nel cap.~\ref{sec:test}) \\
\bottomrule
\end{tabular}
\end{center}
...
solo locale sul dispositivo. I binari degli allegati non sono in nessuna delle
due: stanno su Google Drive, e \texttt{allegati.storage\_path} ne conserva
l'identificativo.
...
\begin{tikzpicture}[node distance=1.0cm and 1.5cm]
  \node[tabella] (ripassi) {%
    \textbf{ripassi}\\[2pt]
    \underline{id} : uuid\\
    user\_id : uuid $\rightarrow$ auth.users\\
    titolo : text\\
...
\end{tikzpicture}}
\caption{Le tre tabelle Postgres (RLS attiva su tutte), la tabella di sola
cache sul dispositivo, e il legame con il file su Drive. Il tratteggio segna
ci\`o che non vive su Postgres.}
\end{figure}
...
npm test           # 232 test su 20 suite (jest-expo)
