...
test sulla logica di calcolo (118 test) e ha superato un audit di sicurezza
...
\texttt{src/model/ripassiLogic.ts} & classificazione attivi/storico e ordinamento totale \\
\texttt{src/model/errorMessages.ts} & errori tecnici $\rightarrow$ messaggi italiani per categoria \\
\texttt{src/model/retry.ts} & backoff esponenziale sui soli errori transitori \\
...
\texttt{src/controller/useConnettivita.ts} & stato online/offline (NetInfo) per la View \\
...
\texttt{src/**/\_\_tests\_\_/} & 118 test jest-expo sulla logica pura \\
\bottomrule
\end{tabular}
\end{center}
...
italiano, con un flag \texttt{ritentabile}. I test verificano non solo la
categoria ma anche che il testo tecnico \emph{non} compaia nel messaggio
finale. Nessun punto dell'app mostra pi\`u \texttt{e.message} grezzo.
...
download allegato & s\`i & idempotente \\
\textbf{crea ripasso} & \textbf{no} & insert non idempotente \\
\textbf{upload allegato} & \textbf{no} & crea file Drive + riga nuovi \\
\bottomrule
\end{tabular}
\end{center}

Le due esclusioni sono deliberate e sono il punto meno ovvio del capitolo. Un
errore di rete pu\`o significare ``la richiesta \`e arrivata ma la risposta si
\`e persa'': ritentare creerebbe un secondo ripasso, o un secondo file su
Drive con una seconda riga in \texttt{allegati} (il nome su Drive \`e generato
con \texttt{Date.now()}, quindi ogni tentativo produce un file diverso).
Chiedere all'utente di ripremere \`e meno grave di un duplicato silenzioso.
